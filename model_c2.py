...
txsm.to_csv("txsmC2-2.csv", encoding="shift_jis")
tysm = pd.DataFrame(tysm)
tysm.to_csv("tysmC2-2.csv", encoding="shift_jis")
...
params={
        'depth': 6, 
        'learning_rate': 0.283339732411781,
        'bagging_temperature': 0.010640053237015685,
        'od_type': 'Iter'}
model = CatBoostClassifier(**params, num_boost_round = 1000, early_stopping_rounds= 300)
...
imps.to_csv("C2imp.csv", encoding="shift_jis")
ytp = pd.DataFrame(y_train_p)
ytp.to_csv("C2tp.csv", encoding="shift_jis")
yivp = pd.DataFrame(y_iv_p)
yivp.to_csv("C2ip.csv", encoding="shift_jis")
yevp = pd.DataFrame(y_ev_p)
yevp.to_csv("C2ep.csv", encoding="shift_jis")
