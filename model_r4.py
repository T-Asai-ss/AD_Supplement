...
# ## input data
# In[2]:
txsmgn =  pd.DataFrame(pd.read_csv(filepath_or_buffer="./tosmgn2.csv", encoding="ms932", sep=","))
...
tysm = smgn['Yo']
txsm = smgn.drop("Yo", axis=1)
tysmdf = pd.DataFrame(tysm.T)
smo = pd.DataFrame(smgn)
smo.to_csv("R4smgn.csv", encoding="shift_jis")
txsm = pd.DataFrame(pd.read_csv(filepath_or_buffer="./txsmR4.csv", encoding="ms932", sep=","))
tysm = pd.DataFrame(pd.read_csv(filepath_or_buffer="./tysmR4.csv", encoding="ms932", sep=","))
...
params={
        'depth': 4,
        'learning_rate': 0.2781978574934286,
        'bagging_temperature': 0.010833674970036573,
        'od_type': 'Iter',
        }
model = CatBoostRegressor(**params, num_boost_round = 1000, early_stopping_rounds= 300)
...
imps.to_csv("R4imp.csv", encoding="shift_jis")
ytp = pd.DataFrame(y_train_p)
ytp.to_csv("R4tp.csv", encoding="shift_jis")
yivp = pd.DataFrame(y_iv_p)
yivp.to_csv("R4ip.csv", encoding="shift_jis")
yevp = pd.DataFrame(y_ev_p)
yevp.to_csv("R4ep.csv", encoding="shift_jis")
