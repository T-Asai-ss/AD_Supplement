...
params={
        'depth': 4,
        'learning_rate': 0.06963650192085206,
        'bagging_temperature': 0.020915234226485665,
        'od_type': 'IncToDec',
        }
model = CatBoostRegressor(**params, num_boost_round = 1000, early_stopping_rounds= 300)
...
imps.to_csv("R1imp.csv", encoding="shift_jis")
ytp = pd.DataFrame(y_train_p)
ytp.to_csv("R1tp.csv", encoding="shift_jis")
yivp = pd.DataFrame(y_iv_p)
yivp.to_csv("R1ip.csv", encoding="shift_jis")
yevp = pd.DataFrame(y_ev_p)
yevp.to_csv("R1ep.csv", encoding="shift_jis")
